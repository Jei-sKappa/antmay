...
| **superseded record** | An ADR or PDR that a later record in the same folder replaced or retired by naming its stem under `supersedes`, moved to that folder's `superseded/` with its content untouched. Location is the state — a decision record carries no status key. |
...
| **entry** | One item in a roadmap index: a slug heading, a sketch, a scope boundary, and the planned behavior the thread opened from it will build, which is the only home of behavior that is settled but not built. The slug is the entry's identifier, and it is pinned once a thread has been opened from it. [suite/shared/references/formats/roadmap-index.md](../suite/shared/references/formats/roadmap-index.md) |
...
| **decision** | Qualify the level. A project decision is an **ADR** or a **PDR**, drafted in a thread's delta and landed at close. A point settled inside a thread and kept there is a **log entry** of type `decision`, and open human intent queued for later is a **pending decision**. |
