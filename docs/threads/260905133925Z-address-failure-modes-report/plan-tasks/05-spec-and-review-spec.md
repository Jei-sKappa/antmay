...
6. Rewrite `## Procedure`: preflight; gather inputs; authoring or amendment pass; audit pass (authoring only); write `spec.md` at the thread root with no frontmatter; append one `event` line to `log.md` with a shell append per `references/formats/log-line.md` stating that the spec was authored or amended; confirm with `Outcome: DONE — Spec written: spec.md` or `Outcome: DONE — Spec amended: spec.md`. State the write boundary: `spec.md` and one `log.md` line; never `adr/`, `docs/adr/`, or `docs/glossary.md`.
...
**Produces:** the in-place amendment rule, stated as: the superseded text is kept, marked superseded, annotated with the date and the reason, and one `log.md` line records the change; the spec `event` line form that the amendment pass keys on. Tasks 6, 9, 10, and 13 restate the rule and the line.
