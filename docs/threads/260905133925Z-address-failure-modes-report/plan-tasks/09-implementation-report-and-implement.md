...
grep -in "editing tool\|file-editing" $I                                       # present
grep -in "verif.*before skipping\|verify against the code\|verifying against the code\|verified against the code" $I   # earlier reports naming the plan
...
- `implement`, when it executes a plan folder, reads earlier reports naming that plan and skips tasks recorded as completed after verifying against the code (AC-9.5).
- `implement` references no roadmap feedback and appends to `log.md` with a shell append, never with an editing tool (AC-9.6, AC-2.2).
