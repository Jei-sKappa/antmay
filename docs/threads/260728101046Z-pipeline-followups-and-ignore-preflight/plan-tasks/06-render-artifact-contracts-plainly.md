### Task 6: Render artifact contracts in plain language
...
**Steps:**
1. Add focused table and formatter cases to `cli/src/thread/artifacts.test.ts` first. Cover every boolean value of `validThread`, `proposal`, `spec`, and `implementationReport`, and every `plan` value (`absent`, `brief`, `strict`, `malformed`); require each phrase to name the concrete file or folder and the structural shape.
...
7. Update artifact, display, runner, and resume expectations for the concrete phrases. Assert both prerequisite and promised-state banners use them, that the `Detail` and `Artifacts:` text share the same table phrases, and that terminal code contains no raw dimension/value formatting.
...
**Consumes:** the queue-helper imports from Task 5 and the reduced runner/resume cursor code from Task 3.
