...
7. Map an unfixable in-run operational defect — including an exhausted commit retry, inaccessible dependency, runtime failure, malformed task discovered only during lazy execution, or structural plan fault — to `BLOCKED` with a diagnosis and no decision bundle. Distinguish a genuine missing-intent question from a defect before selecting the bundle path.
...
**Consumes:** The canonical status-layer rules from Task 1 and the completion-oriented preflight baseline from Task 3.

**Produces:** A factual single-agent progress contract that Task 5 mirrors without importing its topology-specific reply tokens.
