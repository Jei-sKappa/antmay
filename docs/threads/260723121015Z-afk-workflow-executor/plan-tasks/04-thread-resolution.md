...
6. Create `cli/src/test-helpers/git-fixture.ts` exporting `createRepoFixture(options?): Promise<RepoFixture>` — init a disposable Git repository under `fs.mkdtemp`, add and commit ignore rules for the workflow's operational `.pending-decisions/`, `.pending-reviews/`, and `.implementation-runs/` directories, optionally create `docs/threads/<t>/` with seed/decisions content, commit, and return paths plus a cleanup function. Later tasks reuse it for every Git-backed test, so queue files exercise queue gates without entering the Git-boundary status set.
