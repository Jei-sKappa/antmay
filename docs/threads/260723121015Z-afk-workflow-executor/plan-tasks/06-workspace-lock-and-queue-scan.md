...
1. Create `cli/src/workspace/current-checkout.ts` exporting `resolveCurrentCheckoutWorkspace(repoRoot: string): Promise<WorkspaceConfig>` using Task 5's Antmay-owned serializable type. Return `{ strategy: "current-checkout", path: canonicalRoot, execution: { cwd: canonicalRoot, sandbox: "none", branchStrategy: "head" } }`, where `canonicalRoot` is `fs.realpath(repoRoot)`. This module is the single place the run builder obtains execution directory, sandbox choice, branch strategy, and mutable-workspace identity; no settings field selects a strategy.
2. Create `cli/src/state/lock.ts` exporting `acquireWorkspaceLock(stateRoot: string, workspacePath: string, runId: string, now: Date): Promise<LockOutcome>` where `LockOutcome = { ok: true; handle: LockHandle } | { ok: false; lockPath: string; existingRecord: string }`:
   ...
   - if writing or flushing the newly created record fails, close the handle, unlink that exact just-created path best-effort, and rethrow; acquisition must never strand a lock the caller never received;
...
4. Append a concise "Stale workspace locks" section to `cli/README.md`: Antmay never reclaims a lock automatically; after a crash, the user must inspect the printed lock record/path and verify the recorded process is no longer running before manually removing that exact file.
5. Add tests: `cli/src/workspace/current-checkout.test.ts` (realpath resolution through a symlinked repo path and exact cwd/sandbox/branch execution values); `cli/src/state/lock.test.ts` (path is sha256 of the workspace path; mode `0600`; record fields present; injected record-write/flush failure leaves no lock path behind; second acquisition returns contention with the record and path; release with matching token unlinks; release with tampered token leaves the file; release is idempotent); `cli/src/thread/queues.test.ts` (absent dirs empty; direct regular files collected from both queues sorted repo-relative; nested directories and their contents ignored; an unreadable queue directory — e.g. a file where the directory should be, or `chmod 000` — yields `{ ok: false }`; file contents never read, proven with unreadable-content fixtures).

**Files modified:** `cli/src/workspace/current-checkout.ts` (NEW), `cli/src/state/lock.ts` (NEW), `cli/src/thread/queues.ts` (NEW), `cli/README.md`, `cli/src/workspace/current-checkout.test.ts` (NEW), `cli/src/state/lock.test.ts` (NEW), `cli/src/thread/queues.test.ts` (NEW)
...
- Lock-contended commands can point at committed documentation that requires process-death verification before manual removal; no automatic reclaim path exists.
- No settings field or public option selects a workspace strategy; the snapshotted `WorkspaceConfig` supplies canonical identity and the harness execution's cwd/sandbox/branch values (AC-8.4, DR10).
...
**Consumes:** `WorkspaceConfig` from `cli/src/workspace/types.ts` plus the state-root conventions from Task 5 (`afk-locks/` sits beside `afk-runs/` under the same state root; directory mode discipline `0700`).

**Produces:** `resolveCurrentCheckoutWorkspace(repoRoot): Promise<WorkspaceConfig>` from `cli/src/workspace/current-checkout.ts`; `acquireWorkspaceLock(stateRoot, workspacePath, runId, now)` and `LockHandle` from `cli/src/state/lock.ts`; `scanPendingQueues(repoRoot, threadRelPath): Promise<QueueScan>` from `cli/src/thread/queues.ts`.
