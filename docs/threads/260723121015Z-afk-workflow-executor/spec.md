...
**Gate errors (DR44, DR57).** When an existing run's advancement invariant cannot be evaluated — for example an unreadable pending directory — the run durably pauses at the same stage with kind `gate-error`, releases the lock, and exits `2`; a failure to write that pause follows the fatal-checkpoint path (DR27). A queue-read error during initial preflight exits `1` with no run. A queue scan that fails while a parsed-DONE stage's boundary still requires finalization is the one exception: it does not downgrade to `gate-error`. The run preserves the boundary-finalization kind (`git-policy-violation` or `commit-error`) with its original pending-path evidence and records the queue-scan failure as additional diagnostic detail on that same pause, so the boundary stays resumable without another harness invocation (DR57).
...
- A waiting run with empty queues proceeds by kind: a `pending-queues` pause whose finished attempt parsed DONE (its boundary already finalized, the attempt recorded as `done`) continues per the stage's declared queue-resolution behavior — `advance` moves on to the next stage without rerunning the finalized stage, `rerun` starts a fresh attempt of the same stage (DR53); a `pending-queues` pause whose attempt did not parse DONE (the attempt recorded as `waiting`) keeps the unconditional same-stage retry and starts a new attempt at the same stage (DR53); `git-policy-violation`/`commit-error` get boundary finalization without another harness invocation (DR50); every other kind starts a new attempt at the same stage.
...
### FR-11 — Queue gate and classification (DR3, DR41, DR44, DR52, DR57)
...
- **AC-11.6** A queue scan that fails while a parsed-DONE stage's boundary still awaits finalization keeps the `git-policy-violation` or `commit-error` kind and its pending-path evidence, records the scan failure only as added diagnostic detail on that pause, and does not become a `gate-error`; resume repeats the queue scan and finalizes the boundary without another harness invocation once the scan succeeds and queues are empty.

### FR-12 — Git boundaries and commits (DR49, DR50, DR51, DR52, DR54, DR57)
