...
The universal bridge from completion-oriented work back to human judgment. Any completion-oriented operation that cannot proceed safely without settling human intent emits a pending-decision bundle here rather than waiting in chat or inventing an answer. It first completes everything it can derive or fix safely; the queue is only for irreducible judgment.
