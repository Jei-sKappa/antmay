...
description: Reconcile two or more competing candidate drafts of one artifact into the single canonical artifact, folding non-conflicting content automatically and preserving unresolvable subjective conflicts via HTML-comment markers, when the user wants an autonomous merge.
metadata:
...
The decision log captures, per resolved difference, which candidate's content prevailed and a one- or two-sentence rationale. Any conflict the auto merge could NOT confidently resolve is preserved in the canonical artifact body via the `<!-- CONFLICT: -->` marker (below) AND noted in the decision log as unresolved.
