...
- **Loose plan handed to an agent-leaning implementer** — recommend `skills/workflow/plan/adjust-plan-granularity-auto/SKILL.md` with a `stricter` direction during the walk.
- **Strict plan for a simple twenty-minute change** — recommend `skills/workflow/plan/adjust-plan-granularity-auto/SKILL.md` with a `looser` direction during the walk.
...
The plan-content contract from D59 — every task must be **sequential, isolated, independently implementable, and independently reviewable** — applies regardless of granularity per `skills/workflow/plan/plan-loose-auto/SKILL.md` and `skills/workflow/plan/plan-strict-auto/SKILL.md`. The walk enforces it:
...
5. **Settle the finding.** Together, settle as one of:
   ...
   - `accepted` — the finding is genuine and actionable; it will need to be addressed (typically by emitting a `v<N+1>` plan via the upstream Phase 4 plan-* skill, by invoking `skills/workflow/plan/adjust-plan-granularity-auto/SKILL.md` / `skills/workflow/plan/adjust-plan-granularity-interactive/SKILL.md` for granularity shifts, or by re-reviewing an upstream spec via `skills/workflow/review/review-spec-auto/SKILL.md` / `skills/workflow/review/review-spec-interactive/SKILL.md` when source-adherence findings trace back to spec ambiguity). Dumps to the inbox-open review-finding artifact at end-of-session.
...
4. **`## References`** — the plan path under review (absolute path), the decision log path emitted by this same walk (absolute path), the source artifact path (when supplied, absolute path), any related decision logs or prior review-findings by absolute path, and a citation to the relevant Phase 4 plan-* skill (`skills/workflow/plan/plan-loose-auto/SKILL.md` or `skills/workflow/plan/plan-strict-auto/SKILL.md`) when a finding is about the granularity contract.
...
6. **`## Next Actions`** — what to do next for each unresolved finding. Typical actions: emit a `v<N+1>` plan via the upstream Phase 4 plan-* skill, shift granularity via `skills/workflow/plan/adjust-plan-granularity-auto/SKILL.md` / `skills/workflow/plan/adjust-plan-granularity-interactive/SKILL.md`, re-review the upstream spec via `skills/workflow/review/review-spec-auto/SKILL.md` / `skills/workflow/review/review-spec-interactive/SKILL.md` if findings trace back to spec ambiguity, or escalate to implementation via one of the `implement-*` skills once findings are addressed.
