...
description: Read a proposal artifact and write a references-first review report into the proposal's reviews/ folder, covering gaps, risks, ambiguities, and consistency with the thread's decision logs, when the user wants a lightweight autonomous proposal review.
metadata:
...
Read a proposal artifact READ-ONLY and emit a references-first review report into the proposal's own `reviews/` folder. This skill reads the proposal, drafts the report end-to-end, and writes one review record per review run. It does not ask clarifying questions, it does not walk findings with the user one-at-a-time, and it does not commit.
