...
description: Autonomously verify an implementation against its spec's acceptance criteria and write a references-first review record capturing acceptance, constraint, scope, behavior, and test-coverage findings when the user wants the implementation checked without a per-finding walk.
metadata:
