...
2. **Confirm or derive the slug.** Propose a kebab-case slug derived from the user's opening prompt and read it back for confirmation, or ask the user to supply one. Emitted artifacts cannot be renamed — the filename is permanent once written — so settling the slug before writing is cheaper than fixing it later.
...
4. **Capture the UTC stamp.** Compute the 12-character `YYMMDDHHMMSSZ` stamp at write time (two-digit year, month, day, hour, minute, second, trailing `Z` for UTC). Stamp once and reuse — never re-derive after writing.

5. **Assemble and write the artifact.** Compose the proposal body from the user's answers, then write to `docs/threads/<thread>/proposals/<UTC>-<kebab-desc>-proposal.md`. The `-proposal` artifact-type suffix is MANDATORY — no artifact filename may omit it. Create the `proposals/` folder on-demand if it does not yet exist. The proposal body is plain markdown — no YAML frontmatter on the proposal itself.
...
1. **Park as an Inbox item** (PREFERRED for non-blocking side-findings). Write a short markdown record at `docs/threads/<thread>/inbox/open/<UTC>-<kebab-desc>-inbox-item.md` so the side-finding survives without polluting this proposal.
...
The proposal artifact uses this filename grammar:

```text
<YYMMDDHHMMSSZ>-<kebab-desc>-proposal.md
```

The 12-character UTC stamp comes first, followed by a kebab-case description, followed by the literal `-proposal` artifact-type token, followed by `.md`. The artifact-type suffix is MANDATORY — every artifact filename must end with the type token so it is self-identifying when copied or grepped outside its folder.
...
The file lands at `docs/threads/<thread>/proposals/<filename>`. The `proposals/` folder is created on-demand on the first proposal written for the thread.
...
Emitted proposal artifacts are immutable. Once the file is written into `proposals/`, it is part of the thread's reviewable history and is not edited. Revisions emit a new artifact — a new record with a different slug, or a follow-up proposal — rather than rewriting the original.
