...
**Block content.** Because this single-agent topology spawns no subagents and dispatches nothing, the block carries NO subagent-audit lines and NO status token — the same session implements and self-reviews, so there is no separate claimed-versus-verified distinction to record. Each block records the facts named in `## Factual progress records`: the task attempted, the changes made, the verification run and its result, non-blocking concerns verbatim (resolved findings are not restated), and the commit SHA + subject for a committed cycle (else `Commit: none`), plus a next action.
