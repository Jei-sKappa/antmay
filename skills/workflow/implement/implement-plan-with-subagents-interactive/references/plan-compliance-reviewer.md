...
5. **Identify plan-compliance gaps as ACTIONABLE FINDINGS.** Each finding must be concrete (cite the specific plan-task statement and the specific diff observation), not vague ("the diff feels incomplete"). Vague findings are useless to the fix-iteration implementer; the next implementer needs to know what to fix. Findings will also be surfaced to the user live during the orchestrator's walk — keep them precise so the user can evaluate without re-reading the diff.
