...
- **Output path** — ONE `SS-review.md` at the pre-computed scratch path `docs/threads/<thread>/.wip/implement/plans-NNN[-<desc>]/task-NN/SS-review.md`, containing one section per lane. The reviewer writes it ONLY when there is content — any lane `ISSUES` / `BLOCKED` / `NEEDS_CONTEXT`, a lane `PASS` carrying non-blocking concerns, or an out-of-task observation; a both-lanes-clean pass with nothing to report writes no file. Write-once — never overwrite or append a prior dispatch's file.
