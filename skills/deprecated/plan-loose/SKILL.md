...
Read the upstream input, draft numbered tasks with brief 1–3 sentence descriptions per task, self-review before emission, write the plan into its lineage folder, and confirm its path. By default this skill runs end-to-end without walking the user task-by-task, but it honors an invocation that asks it to check in or work through the plan interactively; it does not commit.
...
3. **A decision-log artifact path** — a record carrying one or more settled decisions with sequential `## DP<N>: <Title>` headings. Each settled decision may map to a task (or constrain one) — cite the source log by path + `DP<N>` where the decision is operative in the task description (same-thread references thread-relative, e.g. `discussions/<UTC>-<slug>-decision-log.md DP3`; cross-thread references repo-relative, `docs/threads/<other>/…`).
