...
inputs:
  ...
  report-tether:
    type: string
    required: false
    default: what the plan accomplished
---
# Implement Plan
...
- The state DONE — `DONE` — means the plan task was completed and the implementer has no concerns to surface. The objective and acceptance criteria are met, the self-review pass found nothing to flag, the verification block from the plan task (if any) passes mechanically, and the project's standing required gates — if the project defines any (see `## Commit Policy`) — pass on the changed code. This is the only state that means "ready for next task with no follow-up".
...
::include{root="group", path="partials/task-report-block-shape.md"}
