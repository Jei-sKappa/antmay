...
inputs:
  ...
  disposition-record:
    type: string
    required: false
    default: for an implementation finding, the **fix in a fresh implementation pass is the record**
  rationale-bullet:
    type: string
    required: false
  ...
  frontmatter-lineage-tail:
    type: string
    required: false
    default: ' The only frontmatter a review carries is its lifecycle `status:` map (and only once disposed); lineage between the review, the implementation reference, and the spec lives in the `## References` section, not in metadata.'
---
