...
3. Create `cli/src/commands/run/preflight/resolve-runtime.ts` exporting `resolveRunRuntime`; accept the command-owned scripted-prompt observer and pass it to the lower-level runtime resolver without invoking or defining presentation in the step, resolve exactly one adapter family, probe selected harnesses, and return invoker, runtime identity, observed versions, non-empty process-local version map, and optional scenario path or the existing structured runtime refusal.
...
5. Keep rich composition/runtime rendering, plain binding failures, scripted-prompt presentation through the observer defined by `run.ts`, and all lifecycle responsibilities in the command; leave the later Git and queue safety gates inline for task 3.
...
**Verification:** `npm --prefix cli run test -- src/commands/run.test.ts` exits 0; `npm --prefix cli run check` exits 0; `rg -n 'Preflight [1-9]:' cli/src/commands/run.ts` returns no matches while task 3's later numbered safety blocks remain inline.
...
- Runtime resolution preserves adapter-family selection, selected-harness probing, scripted-prompt observation, immutable runtime identity, and version-map semantics; `run.ts` defines the observer that presents the prompt, while the step imports no renderer and never invokes presentation during preflight.
