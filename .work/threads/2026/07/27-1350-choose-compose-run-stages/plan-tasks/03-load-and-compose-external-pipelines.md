...
5. Begin composition from one freshly inspected concrete artifact state. Walk only the selected entries in document order: use the shared contract evaluators from Task 1 to check each catalog prerequisite and apply its promised transition while preserving unrelated state dimensions; resolve the catalog target against the current simulated state; and record a prepared stage containing the catalog definition plus portable instructions and concrete target.
...
**Consumes:** `STAGE_CATALOG`, `inspectArtifactState`, the pure artifact-contract evaluators, safe target resolution, `resolveDocumentReference`, and the shared raw-name predicate produced by Tasks 1 and 2.
