...
A roadmap index holds one direction: where it is going and the entries that get there. It is a project-level file at `.wip/roadmaps/<yymmddhhmm>-<slug>.md`, where the stamp is the index's creation time in UTC at minute resolution.
...
- When a thread opened from an entry closes, the closing skill writes `Closed: <thread path relative to .wip/threads/> — <one-line outcome>` as the first line beneath that entry's heading.
