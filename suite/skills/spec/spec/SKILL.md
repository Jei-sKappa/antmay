---
name: spec
description: Author the thread's spec and its delta documents, and amend them in place on re-invocation.
...
- The thread's `spec.md`, when the file exists — the spec as it stands, which the run amends rather than authors anew.
...
These are the whole of what the spec rests on: the audit pass checks every claim in the draft against the conversation or the log, the seed, and the thread's delta, so no other material feeds it.
...
The spec is not complete while an open question remains. An open question you can answer from the gathered inputs is answered, and its label goes with the answer. One you cannot answer yourself is a human decision: queue it per `## Blocked` and mark it inline pointing at the bundle.

## Amendment pass

When `spec.md` already exists at the thread root, the run amends it and the thread's delta documents together. Author nothing new.
