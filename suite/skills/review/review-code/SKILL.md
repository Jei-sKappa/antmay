...
The thread's `delta/` applies on top of the resolved anchor as a binding constraint source, alongside the project's decision records and the project glossary terms the delta does not redefine; code that contradicts one of those is a finding. Whether such a contradiction is intentional or a finding is classified as `/consult-decisions` instructs.
