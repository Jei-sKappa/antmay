...
Records are append-only: never rewrite or delete an existing record. When a settled decision later changes, append a new `DR<N>` record that names the record it supersedes (e.g. `Context: supersedes DR4 …`) and states the new resolution.
