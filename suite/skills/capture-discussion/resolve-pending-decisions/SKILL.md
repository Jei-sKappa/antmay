...
1. **Frame the next unsettled point live**, in chat, following the `<skill_path>/references/formats/discussion-point.md` format. The point's blocker is the choice to state; its evidence and the surrounding facts are what the reader needs to know; and you build the fork yourself from them — lettered alternatives with a reasoned pick when the decision is genuinely open, or a single practical proposed solution when one path is the sensible default. A free-text suggestion the producer left is material for that framing, never the answer. Take one point per turn; do not batch.
