...
Applies when the invocation carries a ticket reference. Read the ticket before composing any seed field, then check whether a thread already exists for it. The ticket's body is not raw material here — it becomes the seed's genesis narrative as it stands, so what you read is what the thread will hold.
