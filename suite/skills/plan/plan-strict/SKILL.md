...
You write exactly those files: `plan.md` and `plan-tasks/NN-<kebab-slug>.md` inside the plan folder you created. Nothing else you touch is written — the thread's `change.md` and its `delta/`, and the project layer (`docs/adr/`, `docs/pdr/`, `docs/product/`, `docs/architecture/`, `docs/glossary.md`, and the roadmap indexes under `.work/roadmaps/`) — are read here and never written.
...
2. **A `Source:` line** — names the upstream artifact this plan was compiled from, in exactly one of four legal value forms:
   - a **thread-relative pointer** to the upstream artifact within the active thread (e.g. `change.md`, `delta/docs/adr/<stem>.md`);
   - a **repo-relative path** for a project-level artifact (e.g. `docs/adr/<stem>.md`);
...
3. **A Global Constraints block** — the project-wide requirements the plan must honor, copied **verbatim** from the change document's constraints, one line each. When the source is a referenced artifact that states no constraints, or there is no source (`Source: none — raw prompt`), the block still appears and says so explicitly (e.g. `The source states no constraints.`) — never an omitted block.
...
6. **Acceptance criteria** — the observable state of completion. What must be true after the task is done? A bullet list of post-conditions: "function X exists at module Y", "test Z passes", "config K has value V". Acceptance is the externally observable definition of done; verification is the procedure for checking it. A criterion this task delivers from the change document's acceptance checklist is carried across by quoting it **verbatim** — never numbered, never labelled with an identifier, never paraphrased — so the plan, the implementation report and the review all match on the same wording; the task's own post-conditions sit alongside the quoted lines.
...
- **Pointers carry their own frame.** A pointer from one plan file to another is relative to the plan folder (`plan-tasks/01-…md`); a pointer to a thread file is thread-relative (`change.md`, `delta/docs/adr/<stem>.md`); a project-level pointer is repo-relative (`docs/adr/<stem>.md`). The plan cites no other thread.
...
1. **Coherence** — does the plan, executed end-to-end, actually achieve the primary input's goal? Does completing every plan task satisfy the change document's goal and every line of its acceptance checklist, or, where a referenced artifact is the form, elaborate what it asks for into a complete implementable sequence?
...
A task that would rest on a delta document or a decision of the change document you find wrong is one of these decisions: planning does not proceed on that task, and the document is corrected before it does. An unnoticed conflict between the plan's material and a project decision record or a project glossary term is another: classify it as `/consult-decisions` instructs, and queue it rather than overriding the project record.
