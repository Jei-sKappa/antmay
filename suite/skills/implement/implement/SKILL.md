...
- The thread's `change.md`, when the file exists — the change document, whose acceptance checklist the implementation answers to.
...
The report's acceptance rows are drawn from the change document's acceptance checklist, one row per criterion quoted verbatim, and from the verification the run recorded per task, which supplies the method and the evidence each row names. When the thread holds no change document, the criteria the plan or the input stated take their place.
