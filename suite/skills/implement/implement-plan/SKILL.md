...
One append-only block per attempted plan task lives in the run workspace's `progress.md` (see `## Run workspace`); the progress file alone carries the block. Chat output carries only a one-line summary per task. The progress blocks and the git history together are the audit trail. Each block records:
...
The report's acceptance rows are drawn from the change document's acceptance checklist, one row per criterion quoted verbatim, and from the verification the run recorded per task, which supplies the method and the evidence each row names. When the thread holds no change document, the criteria the plan stated take their place.
...
**Write boundary.** You write the project's code, tests, configuration and living documentation within this implementation's scope, plus this invocation's implementation folder with its `report.md` and its `.runs/`. You write nothing in the project layer — `docs/adr/`, `docs/pdr/`, `docs/product/`, `docs/architecture/`, `docs/glossary.md` and `.work/roadmaps/` — and nothing in `change.md` or `delta/`, `plans/`, other implementation folders or any other thread; all of those are read here and never written.
...
- **Audit trail.** The task's progress block notes that the commit failed N times and what was fixed; the retries stay visible there and nowhere else.
...
When the plan itself needs revision — it calls for an outdated approach, a target file has gone, a whole task rests on a wrong premise — this run's mandate is to surface the fault and stop, per `## Deviations`. A **change-document fault** (the plan faithfully implements an ambiguous or incomplete change document) is fixed by amending the change document and writing a fresh plan from it. A **plan fault** (the plan is wrong while the change document is sound) is fixed by running the plan check against the plan folder, or by writing a fresh plan. Either way the corrected plan is handed to a fresh run.
