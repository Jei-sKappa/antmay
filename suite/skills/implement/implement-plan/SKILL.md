...
The report's acceptance rows are drawn from the spec's acceptance checklist, one row per criterion quoted verbatim, and from the verification the run recorded per task, which supplies the method and the evidence each row names. When the thread holds no spec, the criteria the plan stated take their place.
