...
2. In the same suite, inject completed Git results to prove exit `0` means coverage, exit `1` means missing coverage, and exit `128` from `check-ignore` is returned as a Git error rather than rendered as missing coverage. Inject a non-zero `ls-files` result and a rejected runner using `GitSpawnError` to prove the tracked-content probe and a spawn failure follow the same fail-closed path.
3. Assert the rendered refusal's complete structure: the explanation that Antmay skills write the directories and a later Git boundary would fail; the missing-coverage group first; only the failing repository-wide `docs/threads/**/<workspace>/` rules, named against the resolved repository root the check was given; the tracked file paths exactly as emitted; a copyable `git rm -r --cached -- ...` correction over only the affected workspace directories; and the instruction to commit. Include one ignore-covered tracked workspace, one unignored tracked workspace, and one passing workspace to prove independent classification and omission. Add two single-kind cases — one failing ignore coverage only, one failing tracked content only — each asserting that the group with no failures is absent from the message rather than rendered empty.
...
- Directory coverage, tracked content, aggregation, independent failure grouping, `check-ignore` exit `128`, a non-zero `ls-files` result, and spawn failure are each covered by a focused test that was observed failing before implementation and passing afterward.
- The result message is complete and actionable without command-specific wording: its rules block is named against the resolved repository root, and a failure group with no members is absent rather than empty.
